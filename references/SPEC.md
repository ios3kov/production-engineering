...
Inventory → isolated text snapshot → first-party secret rules + vendor static adapters → normalized report → output renderers. Check status is independent of finding severity. Static pass is independent of release readiness. Workflow documents own spec/task/evidence/fix cycles.
