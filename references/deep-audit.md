...
```json
{"required":["static","history","http","browser","axe","lighthouse","semgrep","zap"],"fail_on":"high"}
```
...
`data` contains the actual tool output. Example structure is not evidence. Generate the bundle in the same trusted CI job as the tools; never relabel old results with a fresh date or digest. Scope digest covers admitted source files, not deployment identity, dependencies, excluded/generated files or browser state. Pin and verify deployment/build identity separately. Reports expire after 24 hours; more than 5 minutes into the future is rejected. Scope, target and all report origins must match; duplicate tools are rejected. This binding does not authenticate a report or prove its coverage.
